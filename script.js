    ...
    .map((show) => {
      ...
      return `
        <article class="show-card">
          <div class="show-date">
            <span class="month">${new Intl.DateTimeFormat("en-US", { month: "short" }).format(new Date(show.date + "T00:00:00"))}</span>
            <span class="day">${new Intl.DateTimeFormat("en-US", { day: "numeric" }).format(new Date(show.date + "T00:00:00"))}</span>
            <span class="weekday">${new Intl.DateTimeFormat("en-US", { weekday: "short" }).format(new Date(show.date + "T00:00:00"))}</span>
          </div>
